...
void setup() {
  ...
  pinMode(waitPin, OUTPUT);  // HIGH = enlighten target;  LOW = SETUP
  pinMode(readyPin, OUTPUT);  // HIGH = enlighten target;  LOW = SETUP
...
//Serial.println("Use buttons to point reflection to target");
      finalAz = targetAzimuth + (az - targetAzimuth) / 2;
      finalAlt = targetAltitude + (el - targetAltitude) / 2;    
    }
...
// moveTo = non blocking
    ...
    azimuthStepper.runToNewPosition(stepsPerRevolution * -finalAz / 360.0);
    elevationStepper.runToNewPosition(stepsPerRevolution * -finalAlt / 360.0);
